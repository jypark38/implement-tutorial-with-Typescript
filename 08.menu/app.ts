...
function displayCategoryBtns(menu: menu[]) {
  const categories = menu.reduce(
    (values: string[], item: menu) => {
  ...
  filterButtons.forEach((btn) => {
    btn.addEventListener("click", (event: MouseEvent) => {
      const category = (event.currentTarget as HTMLButtonElement).dataset.id;
      const filteredMenu = category === "all" ? menu : menu.filter((item) => category === item.category);
